...
import com.oocourse.specs3.exceptions.AppRunnerFailedMessageException;
import com.oocourse.specs3.exceptions.AppRunnerInstantiationException;
import com.oocourse.specs3.exceptions.AppRunnerRunException;
import com.oocourse.specs3.exceptions.InputArgumentException;
...
public class AppRunner {
    ...
    private static final HashMap<InstructionType, InputArgumentParser> PARSER_HASH_MAP
        ...
        put(InstructionType.CONNECTED_BLOCK_COUNT, NONE_PARAMETER_PARSER);
        put(InstructionType.LEAST_TICKET_PRICE, DOUBLE_PARAMETER_PARSER);
        put(InstructionType.LEAST_TRANSFER_COUNT, DOUBLE_PARAMETER_PARSER);
        put(InstructionType.LEAST_UNPLEASANT_VALUE, DOUBLE_PARAMETER_PARSER);
    }};
    ...
    private final Class<? extends RailwaySystem> graphClass;
    private final Constructor<? extends Path> pathConstructor;
    private final Constructor<? extends RailwaySystem> graphConstructor;
    private final RailwaySystem railwaySystem;
    ...
    private final HashMap<InstructionType, ArgumentRunner> RUNNER_HASH_MAP
        ...
        put(InstructionType.CONNECTED_BLOCK_COUNT, AppRunner.this::runAsConnectedBlockCount);
        put(InstructionType.LEAST_TICKET_PRICE, AppRunner.this::runAsLeastTicketPrice);
        put(InstructionType.LEAST_TRANSFER_COUNT, AppRunner.this::runAsLeastTransferCount);
        put(InstructionType.LEAST_UNPLEASANT_VALUE, AppRunner.this::runAsLeastUnpleasantValue);
    }};
    ...
    private AppRunner(
            ...
            Class<? extends RailwaySystem> graphClass,
        ...
        this.railwaySystem = this.graphConstructor.newInstance();
     ...
     * @return 路径容器类型
     */
    public Class<? extends RailwaySystem> getGraphClass() {
        return graphClass;
    }
     ...
     * @return 路径容器类构造函数
     */
    private Constructor<? extends RailwaySystem> getGraphConstructor() {
     ...
     * @return 路径容器
     */
    private RailwaySystem getRailwaySystem() {
    ...
    private RunnerResult runAsPathAdd(InstructionType type, List<Object> arguments) throws Exception {
        ...
        int result = railwaySystem.addPath(path);
    ...
    private RunnerResult runAsPathRemove(InstructionType type, List<Object> arguments) throws Exception {
        ...
        try {
            int result = railwaySystem.removePath(path);
            return new RunnerResult(
                    String.format("Ok, path id is %s.", result),
                    new HashMap<String, Object>() {{
                        put("path_id", result);
    ...
    private RunnerResult runAsPathRemoveById(InstructionType type, List<Object> arguments) throws Exception {
        ...
        try {
            railwaySystem.removePathById(pathId);
    ...
    private RunnerResult runAsPathGetId(InstructionType type, List<Object> arguments) throws Exception {
        ...
        try {
            int result = railwaySystem.getPathId(path);
            return new RunnerResult(
                    String.format("Ok, path id is %s.", result),
                    new HashMap<String, Object>() {{
                        put("path_id", result);
    ...
    private RunnerResult runAsPathGetById(InstructionType type, List<Object> arguments) throws Exception {
        ...
        try {
            Path path = railwaySystem.getPathById(pathId);
    ...
    private RunnerResult runAsContainsPath(InstructionType type, List<Object> arguments) throws Exception {
        ...
        boolean contains = railwaySystem.containsPath(path);
        String message;
        if (contains) {
            message = "Yes.";
        } else {
    ...
    private RunnerResult runAsContainsPathId(InstructionType type, List<Object> arguments) {
        ...
        boolean contains = railwaySystem.containsPathId(pathId);
        String message;
        if (contains) {
            message = "Yes.";
        } else {
    ...
    private RunnerResult runAsDistinctNodeCount(InstructionType type, List<Object> arguments) {
        int count = railwaySystem.getDistinctNodeCount();
        return new RunnerResult(
    ...
    private RunnerResult runAsComparePaths(InstructionType type, List<Object> arguments) throws Exception {
        ...
        try {
            Path xPath = railwaySystem.getPathById(xPathId);
            Path yPath = railwaySystem.getPathById(yPathId);
    ...
    private RunnerResult runAsContainsNode(InstructionType type, List<Object> arguments) {
        ...
        String message;
        boolean contains = this.railwaySystem.containsNode(nodeId);
        if (contains) {
            message = "Yes.";
        } else {
            message = "No.";
    ...
    private RunnerResult runAsContainsEdge(InstructionType type, List<Object> arguments) {
        ...
        String message;
        boolean contains = railwaySystem.containsEdge(fromNodeId, toNodeId);
        if (contains) {
            message = "Yes.";
        } else {
            message = "No.";
    ...
    private RunnerResult runAsIsNodeConnected(InstructionType type, List<Object> arguments) throws Exception {
        ...
        String message;
        try {
            boolean is_connected = railwaySystem.isConnected(fromNodeId, toNodeId);
            if (is_connected) {
                message = "Yes.";
            } else {
                message = "No.";
    ...
    private RunnerResult runAsShortestPathLength(InstructionType type, List<Object> arguments) throws Exception {
        ...
        try {
            int length = this.railwaySystem.getShortestPathLength(fromNodeId, toNodeId);
            return new RunnerResult(
    ...
    /**
     * CONNECTED_BLOCK_COUNT指令
     *
     * @param type      指令类型
     * @param arguments 解析后的参数列表
     * @return 运行结果
     */
    private RunnerResult runAsConnectedBlockCount(InstructionType type, List<Object> arguments) {
        int count = this.railwaySystem.getConnectedBlockCount();
        return new RunnerResult(
                String.format("Ok, connected block count is %s.", count),
                new HashMap<String, Object>() {{
                    put("count", count);
                }}
        );
     ...
     * @throws Exception 运行异常
     */
    private RunnerResult runAsLeastTicketPrice(InstructionType type, List<Object> arguments) throws Exception {
        int fromNodeId = (Integer) arguments.get(1);
        int toNodeId = (Integer) arguments.get(2);

        try {
            int price = this.railwaySystem.getLeastTicketPrice(fromNodeId, toNodeId);
            return new RunnerResult(
                    String.format("Ok, least price is %s.", price),
                    new HashMap<String, Object>() {{
                        put("price", price);
                    }}
            );
     ...
     * @throws Exception 运行异常
     */
    private RunnerResult runAsLeastTransferCount(InstructionType type, List<Object> arguments) throws Exception {
        int fromNodeId = (Integer) arguments.get(1);
        int toNodeId = (Integer) arguments.get(2);

        try {
            int count = this.railwaySystem.getLeastTransferCount(fromNodeId, toNodeId);
            return new RunnerResult(
                    String.format("Ok, least transfer count is %s.", count),
                    new HashMap<String, Object>() {{
                        put("count", count);
                    }}
            );
     ...
     * @throws Exception 运行异常
     */
    private RunnerResult runAsLeastUnpleasantValue(InstructionType type, List<Object> arguments) throws Exception {
        int fromNodeId = (Integer) arguments.get(1);
        int toNodeId = (Integer) arguments.get(2);

        try {
            int value = this.railwaySystem.getLeastUnpleasantValue(fromNodeId, toNodeId);
            return new RunnerResult(
                    String.format("Ok, least unpleasant value is %s.", value),
                    new HashMap<String, Object>() {{
                        put("value", value);
                    }}
            );
